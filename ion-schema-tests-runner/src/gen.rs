use crate::gen::util::*;
use crate::test_case_model::{TestCaseDetails, TestCaseVec};
...
pub fn generate_test_root_module(root_dir_path: &Path, exclude: ExcludeFilter) -> TokenStream {
...
fn generate_submodule_content_for_path(ctx: Context) -> TokenStream {
    ...
    if path.is_dir() {
        ...
        let inner_ts_vec = inner_ts_vec;
        util::join_token_streams(inner_ts_vec)
...
fn generate_test_cases_for_file(ctx: Context) -> TokenStream {
    ...
    /// Macro is only for the generate_test_cases_for_file function, so it makes some assumptions about
    /// what variables are in scope
    macro_rules! maybe_ignore {
        ($test_case_name:expr) => {
            if (ctx.exclude)(format!("{}::{}", path_to_mod_name(ctx.current_dir.as_path()), util::escape_to_ident(&$test_case_name))) {
    ...
    test_case_tokens.push(quote!(
        ...
        fn should_be_a_valid_schema() {
            __get_schema_system().load_schema(#schema_id_literal).unwrap();
        }
    ));
    ...
    let test_cases_ts = util::join_token_streams(test_case_tokens);
...
fn generate_preamble(root_dir_path: &Path) -> TokenStream {
    ...
    quote! {
        ...
        fn __get_schema_system() -> ion_schema::system::SchemaSystem {
        ...
        fn __get_schema_system_with_additional_schema(id: &str, content: &str) -> ion_schema::system::SchemaSystem {
        ...
        fn __assert_schema_validity(schema_isl: &str, expect_valid: bool) -> Result<(), String> {
            let mut schema_system: ion_schema::system::SchemaSystem = __get_schema_system_with_additional_schema(
                "__schema__",
                schema_isl
            );
        ...
        fn __assert_type_is_invalid(type_def: &str) -> Result<(), String> {
            ...
            let mut schema_system: ion_schema::system::SchemaSystem = __get_schema_system_with_additional_schema("__invalid_type__", &*schema_isl_text);
        ...
        fn __assert_value_validity_for_type(value_ion: &str, schema_id: &str, type_id: &str, expect_valid: bool) -> Result<(), String> {
            let schema = __get_schema_system().load_schema(schema_id).unwrap();
...
mod util {
    ...
    pub fn join_token_streams(ts_vec: Vec<TokenStream>) -> TokenStream {
        ...
        ts_vec
            .iter()
            .for_each(|ts| joined_token_stream.extend(ts.to_owned().into_iter()));
        joined_token_stream
    }
...
// we have this test case. Any problems with the generated code will be caught by the invocation
// of the macro.
#[cfg(test)]
mod generator_does_not_panic {
    use super::generate_test_root_module;
    use std::path::Path;
    use std::rc::Rc;
    ...
    fn test(path: &str) {
        let _ts: proc_macro2::TokenStream =
            generate_test_root_module(Path::new(path), Rc::new(|_| false));
    }
}
